...
vec4 hook()
{
	...
	vec2 c = HOOKED_tex(vec2(tc12.x, tc0.y)).xy * w12w0;
	c += HOOKED_tex(vec2(tc0.x, tc12.y)).xy * w0w12;
	c += HOOKED_tex(tc12).xy * w12w12;
	c += HOOKED_tex(vec2(tc3.x, tc12.y)).xy * w3w12;
	c += HOOKED_tex(vec2(tc12.x, tc3.y)).xy * w12w3;
